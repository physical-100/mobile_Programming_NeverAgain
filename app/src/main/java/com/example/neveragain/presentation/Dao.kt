...
interface DAO{
    ...
    @Query("SELECT * FROM restaurant WHERE id IN ( SELECT restaurant_id FROM custom_list WHERE user_id = :user_id AND is_white = :is_white )")
    ...
    @Query("SELECT * FROM order_content WHERE user_id = :user_id")
    suspend fun getUserOrderList(user_id: Long): List<Order>
    @Query("SELECT * FROM order_content WHERE restaurant_id = :restaurant_id")
    suspend fun getRestaurantOrderList(restaurant_id: Long): List<Order>
